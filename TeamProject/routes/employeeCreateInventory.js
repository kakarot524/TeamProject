...
app.get('/',function(req,res){
	var emess = "";
	res.render('employeeCreateInventory',{emess: emess, user:req.session.usernameCookie});
});
...
app.post('/',function(req,res){
	...
	if ((name.indexOf('\'') > -1) || (name.indexOf('"') > -1) || (name.indexOf(';') > -1) || (name.indexOf('`') > -1)) {
		...
		res.render('employeeCreateInventory',{emess: emess});
					...
					if(err) {
						...
						con.query(queryString_stock, function(err, rows, fields){
							if(err) {
								...
								res.redirect('employeeMenu#!/dashboard'); // needs to be changed to employee
