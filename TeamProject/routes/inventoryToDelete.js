...
app.get('/',function(req,res){
	 ...
	 if(req.session.usernameCookie == null && req.session.roleidCookie == null) {
		...
		if(req.session.roleidCookie == 2) {
			...
			"AND TABLE_NAME NOT LIKE 'role' AND TABLE_NAME NOT LIKE 'deleted_user' ORDER BY table_name ASC ";
			...
			con.query(tableQuery, function(err, rows, fields){
				if(err) {
					...
					obj = rows;
					res.render('inventoryToDelete',{data: obj,user:req.session.usernameCookie});
			
				}//elseif
			});//quer
		} else {
			res.render('inventoryToDelete', {user:req.session.usernameCookie});
